...
dotenv.config();
const { CLIENT_ID, CLIENT_SECRET, WEBHOOK_SECRET_TOKEN, OAUTH_PORT = "4000", } = process.env;
if (!CLIENT_ID || !CLIENT_SECRET || !WEBHOOK_SECRET_TOKEN) {
    console.error("Ensure CLIENT_ID, CLIENT_SECRET, and WEBHOOK_SECRET_TOKEN are set in .env");
    process.exit(1);
}
...
(async () => {
    try {
        phoneClient.webEventConsumer.event("phone.recording_completed", async ({ payload }) => {
            try {
